...
describe('DashboardPageComponent', () => {
  ...
  it('builds chart data, tooltip details, and resource-cost states for daily points', async () => {
    ...
    expect(chartData.datasets[0]?.pointBackgroundColor).toBe('#38bdf8');
    expect(chartData.datasets[1]?.pointBackgroundColor).toBe('#34d399');
    expect(chartData.datasets[2]?.pointBackgroundColor).toBe('#fb7185');
    expect(chartData.datasets[3]?.pointBackgroundColor).toBe('#c084fc');
    expect(chartData.datasets[0]?.pointBorderColor).toEqual(['#34d399', '#fbbf24', '#fb7185']);
